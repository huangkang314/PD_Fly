function kine_table_grouped = plot_grouped_kineParas_onlyBoxplot(kine_table)
...
kineMean_group = zeros(n_group, n_paras);
boxPlot_data_all = [];
colors = [];
hold on
...
for ig = 1:n_group
    kineParas_selected = kine_matrix(strcmp(kine_table.group_label, uniqGroup_label{ig}), :);
    kineMean = mean(kineParas_selected);
    kineStderr = std(kineParas_selected, 0, 1) / sqrt(size(kineParas_selected, 1));
    kineStderrvals = [kineMean + kineStderr, kineMean(end:-1:1) - kineStderr(end:-1:1)];
    
    kineMean_group(ig, :) = kineMean;
    
    % box plot data
    n_sample = size(kineParas_selected, 1);
    boxPlot_data = zeros(n_sample*n_paras, 4);
    figure(fh1);
    hold on
    for isa = 1:n_sample
        boxPlot_data((isa-1)*n_paras+1 : isa*n_paras, 1) = ig*ones(1, n_paras);
        boxPlot_data((isa-1)*n_paras+1 : isa*n_paras, 2) = (1:n_paras)+1/(n_group+1)*(ig-0.5);
        boxPlot_data((isa-1)*n_paras+1 : isa*n_paras, 3) = kineParas_selected(isa, :);
        boxPlot_data((isa-1)*n_paras+1 : isa*n_paras, 4) = (1:n_paras);
        plot(1:n_paras, kineParas_selected(isa, :), makers{ig}, 'Color', [clrg(ig, :), 0.4], 'MarkerSize', 3, 'LineWidth', 1)
    end
    
    plot_avgerr(kineParas_selected, clrg(ig, :), 1);
    %     plot(1:n_paras,  mvMean, 's-', 'Color', [clrg(6, :), 0.4], 'MarkerSize', 3)
    
    figure(fh2);
    color = repmat(clrg(ig, :), n_sample*n_paras, 1);
...
%     legend_label{ig*2-1, 1} = uniqGroup_label{ig};
%     legend_label{ig*2, 1} = '';
    ...
    colors = [colors; repmat(clrg(ig, :), n_sample*n_paras, 1)];
    
    eval(['kine_table_grouped.', uniqGroup_label{ig}, ' = transpose(kineParas_selected);'])
...
%                     text((start+stop)/2, 0.2+max(boxPlot_data_all(:, 3)), '***')
...
figure(fh1)
xlabel('Kinematic Parameters'); ylabel('Kinematics Values (Z-scored)')
set(gca,'XTick', 1:n_paras, 'XTickLabel', paras_name, 'TickLabelInterpreter','none', 'XTickLabelRotation', 45, 'Box',  'off', 'TickDir', 'out');
xlim([0, n_paras+2]);
% ylim([0, 0.20]);
...
xlabel('Kinematic Parameters'); ylabel('Kinematic Values (Zscored)')
set(gca,'XTick', 1/(n_group+1)*((n_group+1)/2-0.5)+(1:n_paras), 'XTickLabel', paras_name, ...
    'TickLabelInterpreter','none', 'XTickLabelRotation', 45, 'Box',  'off', 'TickDir', 'out');
xlim([0, n_paras+2]);
% ylim([0, max(boxPlot_data_all(:, 3))]);

kine_table_grouped = struct2table(kine_table_grouped);
